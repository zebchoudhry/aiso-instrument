...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!supabase || !(process.env.SUPABASE_URL && (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY))) {
    if (req.method === 'POST') {
      return res.status(201).json({
        id: null,
        saved: false,
        message: 'Database not configured; audit not persisted.',
      });
    }
  ...
  if (req.method === 'POST') {
    try {
      const body = req.body as {
        ...
        findings?: object;
        fixLibrary?: object;
        clientBriefing?: object;
        queryPack?: object;
        userId?: string;
      };

      if (!body?.auditResult) {
        return res.status(400).json({ error: 'auditResult required' });
      }

      const scores = (body.auditResult as any)?.summary?.scores ?? null;
      ...
      const { data, error } = await supabase
        ...
        .insert({
          url: body.url ?? '',
          name: body.name ?? '',
          extraction_data: body.extractionData ?? null,
          audit_result: body.auditResult,
          scores,
          findings: body.findings ?? null,
          fix_library: body.fixLibrary ?? null,
          client_briefing: body.clientBriefing ?? null,
          query_pack: body.queryPack ?? null,
      ...
      if (error) {
        console.error('[audits] Insert error:', error);
  ...
  if (req.method === 'PATCH') {
    const patchBody = req.body as { id?: string; findings?: object; fixLibrary?: object; clientBriefing?: object; verifications?: object; queryPack?: object };
    ...
    try {
      ...
      if (id) {
        ...
        return res.status(200).json({
          ...
          extractionData: data.extraction_data ?? null,
          findings: data.findings,
          fixLibrary: data.fix_library,
          clientBriefing: data.client_briefing,
          verifications: data.verifications ?? null,
          queryPack: data.query_pack ?? null,
      ...
      let query = supabase
        .from('audits')
        .select('id, url, name, scores, created_at', { count: 'exact' })
