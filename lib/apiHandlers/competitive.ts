...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!yourUrl || !competitorUrl) {
      return res.status(400).json({ error: 'Missing required fields', details: 'yourUrl and competitorUrl are required' });
    }
    const analysis = await CompetitiveAnalysisService.compareWebsites(yourUrl, competitorUrl);
    return res.status(200).json(analysis);
  } catch (err) {
    console.error('competitive analysis error:', err);
    return res.status(500).json({ error: 'Competitive analysis failed', details: err instanceof Error ? err.message : String(err) });
  }
}
